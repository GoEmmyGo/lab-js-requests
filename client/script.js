...
sayHelloButton.addEventListener(`mouseout`, changeButton)
...
const queryTest = () => {
    axios.get(`http://localhost:3000/query-test/?garbage-day`)
    .then((res) => {
        console.log(res.data)
    })
}
...
/*
    ...
    sent an empty query in my console

    2: Send more than 1 query on the URL -- what happened? 

    strung together both queries
*/
...
const createFood = (event) => {
    ...
    const foodInput = document.getElementById('food-type')
    const body = {
        newFood: foodInput('')
    }
    ...
    createFood.appendChild(`#foodInput`)
}
